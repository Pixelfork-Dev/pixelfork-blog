 ...
 *
 *   npm run cover -- <slug> "<subject>" [--out path.webp]
 *
...
const prompt = `Create a new blog cover illustration. Match the art style, lighting, materials and palette of the reference images exactly, but with a new subject.
...
await fs.appendFile(log, `${header}${new Date().toISOString()},${slug},${MODEL},${usage.promptTokenCount ?? 0},${imageTokens},${cost.toFixed(4)}\n`);
